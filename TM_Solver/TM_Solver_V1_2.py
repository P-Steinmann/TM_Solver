...
Program Version 1.2.1                                                                                      Author: Paul Steinmann''')

##### TEST REFLECTIVITY AND FIELD FOR BRAGG MIRROS; IT LOOKS OF ???? ''''
...
class ROOT(Tk):
    ...
    # Main Window Setup
        ...
        self.material_list = ('Gold', 'Silver', 'Air','BK7', 'GaAs', 'AlAs', 'InAs', 'Si', 'TiO2', 'SiO2', 'Si3N4','Chrom', 'Copper', 'Water', 'FusedSilica', 'MgF2', 'h-BN', 'MoSe2-1L', 'WS2-1L') # 12
    ...
    # Frame for Simulation/Calculation Controls
        ...
        self.mode_RvsW_AOI  = ttk.Combobox(self.frame_Sim, textvariable=self.var_RvsW_AOI, values= ['Wavelength', 'Angle of Incidence'], state="readonly", width = 17)
        ...
        self.mode_Eval  = ttk.Combobox(self.dummyframeS, textvariable=self.var_Eval, values= ['Reflectivity', 'E_Field'], state="readonly", width = 10)
    ...
    def R_vs_wavelength(self): # Can this be faster ? /AOI
        ...
        if mode == "Angle of Incidence":
            ...
            self.matplotCanvas(self.frame_ShowSim, A_list, R_list, xlabel = 'AOI [deg°]', ylabel = 'Reflectivity', color = "tab:red", figsize=(4.9, 3.05))
            self.label_RvsW = Label(self.frame_Sim, text = "Finished !", width = 18, bg = 'grey90').grid(row = 4, column = 2, columnspan = 1)
    ...
    def OnDoubleClick(self, event): # Adjusted from: https://stackoverflow.com/questions/18562123/how-to-make-ttk-treeviews-rows-editable
        ...
        y = root.winfo_y()
        win = Toplevel()
        win.geometry("%dx%d+%d+%d" % (480, 58, x + 5, y + 90))
    ...
    def E_Field_Calculation(self, thickness, epsilon, wavelength, Precision, Progressbar = True, kz = 0):
        ...
        kx = np.sqrt(epsilon*k0**2 - kz**2, dtype=np.complex128) #test
