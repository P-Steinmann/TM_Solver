'''
    ...
    Version: 1.4 19.02.2022
        ...
        Version Added: 1.4.2
            Added Transmittance and Absorbance in "Compute Reflectivity vs" funcionality
            Added Transmittance and Absorbance in "Compute" funcionality
        
        Version Added: 1.4.3
            Added "Custom Functionality" Button where i can add specific stuff I need
                Need to work on the Refl vs Int e field function !

        Version Added: 1.4.4
            Some Bug fixing for the E_Field Computation .... hopefully works better now.
        
        Version Added: 1.4.5 -> 1.5
            Fixed the initial Error with mode_params
            Fixed The n+ik functionality for gold, reworked it in general
            Added Wavelength range desription to Fused Silica
            Added the Maximum/Integration option for the Efield calculation
        
    Version: 1.5 05.07.2022
    ...
    Progress until V2:
        ...
        Possible Upgrades:
            ...
            #Bug: n+ik doesnt work often if material range is exceeded, eg. for gold WORK ON THIS
            #Add Efield--max/sum option
            
            Add AOI to "Compute" functionality!
            Bug: If a material name is changed in treeview, this has to affect all comboboxes, which is not yet the case
            Add "Transmittance" in the "Compute" simulation
...
Program Version 1.5.0                                                                                      Author: Paul Steinmann''')
...
class ROOT(Tk):
    ...
    # Frame for Simulation/Calculation Controls
        ...
        self.frame_Sim.place(x = 0, y  = 355, width = 600, height = 280)
        ...
        self.label_of = Label(self.dummyframeS, text = 'of', bg = "grey80").grid(row = 0, column = 3, columnspan = 1, padx = 0)
        ...
        # Row 7
        self.button_Custom = Button(self.frame_Sim, text = "Custom Functionality", bd = 3, command = self.CustomF, bg = "grey80")
        self.button_Custom.grid(row = 7, column = 3, padx = 5)
    ...
    def materials(self, Material, wavelength): 
        ...
        def n_FusedSilica(wavelength):
            if wavelength < 210 or wavelength > 6700:
                print("WARNING: Wavelength range for Material exceeded (Gold)")
            w = (wavelength/1000)**2 # For sellmeier equation
    ...
    def load_geometry(self):
        ...
        filename = fd.askopenfilename(title='Open a geometry file', initialdir='Geometries', filetypes=filetypes)
        file = open(filename)
        file_cont = file.read()
    ...
    def R_vs_wavelength(self): # Can this be faster ? /AOI
        ...
        if mode == 'Wavelength':
            ...
            for i in range(len(T_list)):
                absorbance.append(1-T_list[i]- R_list[i])
            self.matplotCanvas(self.frame_ShowSim, [w_list, w_list, w_list], [R_list, T_list, absorbance], xlabel = 'Wavelength [um]',
                               ylabel = 'Reflectivity / Transmittance', color = ["tab:red", "tab:blue", "black"], figsize=(4.9, 3.05), label = ["R", "T", "Absorbance"])
    ...
    def E_field_distribution(self): # ADD X,Y LABEL
        ...
        # Derive Field
        E_Field, x, index = self.E_Field_Calculation(thickness, epsilon, w_centre, 9999)
        ...
        # Static view:
            ...
            E_old = E_Field.max()
            for i in range(191):
                E_new = E_Field*np.exp(-1.0j*np.pi*i/190)
                if E_new.max() > E_old:
                    i_max = i
                    E_old = E_new.max()
            # Absolute Field or norm. Field:
            if is_on == True:
                E_Field = np.real(E_Field*np.exp(-1.0j*np.pi*i_max/190))/abs(E_Field).max()*index.real.max()
                E_Field = abs(E_Field)
            else:
                E_Field = np.real(E_Field*np.exp(-1.0j*np.pi*i_max/190))/abs(E_Field).max()*index.real.max()
    ...
    def RvsParameter(self):
        ...
        if main_mode == "n+ik": # Adjust range for mode:n+ik
            ...
            for i in range(len(descr)):
                if descr[i] == Chosen_Medium:
                    MediaIndex = i
                    counter = counter + 1 # Counter checks for uniques
                    print("Effective Refractive Index of", descr[MediaIndex], "(", r_index_list[MediaIndex],") is calculated")
            stop = False
            w_min = 0
            w_max = 3000
            k = 0
            while stop == False:
                if k > 5000 or w_min >= w_max:
                    break
                print("Estimating available lower wavelength range ...", end = "\r")
                try:
                    sys.stdout = open(os.devnull, 'w') #disable print 
                    self.materials(r_index_list[MediaIndex], w_min)
            ...
            while stop == False:
                ...
                if k > 5000 or w_max <= 0:
                    break
                try:
                    sys.stdout = open(os.devnull, 'w') #disable print
                    self.materials(r_index_list[MediaIndex], w_max)
                    sys.stdout = sys.__stdout__ #enable print
                    stop = True
                except:
                    w_max = w_max - 5
        ...
        print(f"Scanning from {lower} to {upper} in steps of {step}.", end = "\r")
        current = lower
        ...
        while current <= upper: #Iterate over the Parameter range
            ...
            if main_mode == 'Reflectivity':
                ...
                t,r,T,R =  self.T_R_Calculation(thickness, epsilon, wavelength, sqrt(epsilon[0]), sqrt(epsilon[-1]), angle = int(self.entry_AOI.get()))
            ...
            if main_mode == 'E_Field': 
                ...
                E_Field, x, index = self.E_Field_Calculation(thickness, epsilon, wavelength, 9999, Progressbar = False)
                
                # Phase of E_Field is adjusted to show maximum amplitude
                #E_Field = E_Field#/abs(E_Field).max()
                #E = abs(E_Field.real)
        ...
        if main_mode == 'Reflectivity':
            ...
            self.matplotCanvas(self.frame_ShowSim, [L_list, L_list, L_list], [R_list, T_list, absorbance], xlabel = xlabel,
                               ylabel = 'Reflectivity / Transmittance', color = ["tab:red", "tab:blue", "black"], figsize=(4.9, 3.), label = ["R", "T", "Absorbance"])
    ...
    def EorN_selected(self, index, value, op):
        ...
        if mode == "E_Field":
            ...
            self.mode_Para.current(0)
            # Set up window and position it
            win = self.window(230, 60, 165, 535, "Layer where E Field is computed:")
            ...
            var_SumOrMax = StringVar()
            mode_SOM  = ttk.Combobox(win, textvariable= var_SumOrMax, values= ["Maximum","Integration"], width = 15, state="readonly")
            mode_SOM.grid(row = 2, column = 0, pady = 5, padx = 5, columnspan= 1)
            mode_SOM.current(0)

            def UpdateThenDestroy():
                self.Analysed_Medium = var_Mat.get()
                self.SOM = var_SumOrMax.get()
                win.destroy()
    ...
    def CustomF(self):
        win = self.window(230, 70, 165, 535, "Custom Functions:")
        
        var_Med1 = StringVar()
        var_Med2 = StringVar()
        mode_Med1 = ttk.Combobox(win, textvariable= var_Med1, values=self.material_list_para, state="readonly", width = 12)
        mode_Med1.grid(row = 0, column = 1, pady = 5, padx = 3, columnspan= 1)
        mode_Med1.current(0)
        mode_Med2 = ttk.Combobox(win, textvariable= var_Med2, values=self.material_list_para, state="readonly", width = 12)
        mode_Med2.grid(row = 0, column = 2, pady = 5, padx = 3, columnspan= 1)
        mode_Med2.current(0)

        def RvIntE():
            #Get reflectivity stuff over geometry
            #Get Efield over geometry and integrate
            pass
        button_RvsIntE = Button(win, text="R vs Int. Field in Media", command = RvIntE())
        button_RvsIntE.grid(row = 0, column = 0, pady = 3, padx = 3)
        
        canButt = Button(win, text = "Cancel")
        canButt.bind("<Button-1>", lambda c: win.destroy())
        canButt.grid(row = 0, column = 3, pady = 3, padx = 5)
